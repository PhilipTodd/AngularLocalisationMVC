...
var percentApp = angular.module('ng-percent', ['percentage']);

percentApp.directive('ngPercent', ['$filter', '$locale', function ($filter, $locale) {
    return {
        require: 'ngModel',
        link: (scope, element, attrs, controller) => {
            let hardCap, min, max, percentSymbol, ngRequired;
            let active = true;
            let fraction = 2;

            attrs.$observe('ngPercent', (value) => {
                active = (value !== 'false');
                if (active) {
                    reformat();
                } else {
            ...
            attrs.$observe('percentSymbol', (value) => {
                percentSymbol = value;
                reformat();
            });
            ...
            controller.$formatters.push((value) => {
                if (active && [undefined, null, ''].indexOf(value) === -1) {
                    //return $filter('currency')(value, getPercentSymbol(), fraction);
                    return $filter('percentage')(value, fraction, getPercentSymbol());
                }
                return value;
            });
            ...
            function clearValue(value) {
                ...
                //const neg_dummy = $filter('currency')('-1', getPercentSymbol(), fraction);
                const neg_dummy = $filter('percentage')('-1', fraction, getPercentSymbol());
            ...
            function getPercentSymbol() {
                //return percentSymbol === undefined ? $locale.NUMBER_FORMATS.PERCENT_SYM : percentSymbol;
                return percentSymbol === undefined ? '%' : percentSymbol;
            }
        }
    };
}]);
